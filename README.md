...
<title>HAIDEN ANGE — Expert en Visibilité & Croissance Digitale</title>
...
.marquee-track{display:inline-flex; gap:48px; animation:scrollLeft 35s linear infinite; padding-left:100%;}
...
header.scrolled { top: 0; padding: 5px 0; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
...
.nav-links a:hover{color:var(--gold-light); transition: var(--transition);}
...
.menu-toggle{display:none; font-size:22px; color:var(--gold); background:none; border:none; cursor: pointer;}
...
.btn-gold:hover{background:var(--gold-light); transform:translateY(-3px); box-shadow: 0 10px 20px rgba(201,166,89,0.2);}
.btn-outline{border-color:var(--gold); color:var(--gold-light);}
.btn-outline:hover{background:rgba(201,166,89,0.1); transform:translateY(-3px);}
...
.svc-card{background:var(--bg); padding:36px 26px; transition: var(--transition);}
.svc-card:hover{background:var(--bg-2); transform: translateY(-5px); z-index: 2;}
...
.price-table tr td{cursor:pointer; transition: var(--transition);}
...
.price-val i{font-size:12px; opacity:0; transition: var(--transition);}
...
.guarantee-card{background:var(--bg-2); padding:34px 26px; transition: var(--transition);}
...
.why-card{background:var(--bg); padding:38px 28px; transition: var(--transition);}
.why-card:hover{background:var(--bg-2); transform:translateY(-5px);}
...
.step{text-align:left; position:relative; padding-top:8px; transition: var(--transition);}
...
.carousel-track{display:flex; transition:transform .5s cubic-bezier(0.4, 0, 0.2, 1);}
...
.dot2{width:7px; height:7px; border-radius:50%; background:var(--line); cursor:pointer; transition: var(--transition);}
.dot2.active{background:var(--gold); width: 20px; border-radius: 4px;}

/* ---------- FAQ ---------- */
.faq-item{border-bottom:1px solid var(--line);}
.faq-q{display:flex; justify-content:space-between; align-items:center; padding:24px 0; cursor:pointer; font-size:16px; font-weight:500; transition: var(--transition);}
...
.footer-links a:hover{color:var(--gold-light); transition: var(--transition);}
...
.toast{
  ...
  transform:translateY(120%); transition: transform 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275); box-shadow:0 10px 30px rgba(0,0,0,0.4);
}
.toast.show{transform:translateY(0);}
.toast i.icon{color:var(--green); font-size:18px;}
.toast p{font-size:12.5px; color:var(--white); line-height:1.4;}
.toast span{color:var(--gold-light); font-size:11px; font-weight: 600;}
...
.live-visitor {
    position: absolute; top: 20px; right: 20px; font-size: 11px; color: var(--muted); display: flex; align-items: center; gap: 6px;
}
.live-visitor b { color: var(--gold-light); }
</style>
</head>
<body>
...
<section class="hero" id="accueil">
  <div class="container hero-grid">
    ...
    <div class="hero-panel reveal">
      <div class="live-visitor"><span class="live-dot"></span> <b>142</b> personnes consultent l'offre</div>
    ...
    <div class="calc-box reveal">
      <div>
        <div class="calc-select-row">
          <button class="calc-chip active" data-rate="9" data-label="abonnés Instagram"><i class="fa-brands fa-instagram"></i> Abonnés</button>
          <button class="calc-chip" data-rate="4.5" data-label="likes Instagram"><i class="fa-solid fa-heart"></i> Likes</button>
          <button class="calc-chip" data-rate="1.4" data-label="vues TikTok"><i class="fa-brands fa-tiktok"></i> Vues</button>
        </div>
...
function orderVia(desc, price){
  const msg = encodeURIComponent(`Bonjour HAIDEN ANGE, je souhaite commander l'offre suivante :\n\n📦 Service : ${desc}\n💰 Prix : ${price}\n\nMerci de m'indiquer la marche à suivre.`);
...
function updateCalc(){
  ...
  const price = Math.round((qty * activeRate) / 100) * 100;
...
function updateVisitors() {
    const visitorEl = document.querySelector('.live-visitor b');
    let currentVis = parseInt(visitorEl.textContent);
    const change = Math.floor(Math.random() * 5) - 2;
    visitorEl.textContent = Math.max(120, currentVis + change);
}
